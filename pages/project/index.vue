...
useSeoMeta({
  title: "Projects — Shafwan Ilham Dzaky",
  description: "Production systems, internal tools, and open-source contributions. Each project solved real problems with measurable impact.",
  ogTitle: "Projects — Shafwan Ilham Dzaky",
  ogDescription: "Production systems, internal tools, and open-source contributions.",
});
...
const filteredProjects = computed(() => {
  ...
  if (selectedTech.value) {
    result = result.filter((p) => p.tech?.includes(selectedTech.value!));
  }
  ...
  >
    ...
    <aside class="w-full hidden lg:block">
      <div class="flex items-start flex-col justify-between fixed">
        <div class="flex items-start text-white">
          <span class="[writing-mode:vertical-lr] text-2xl font-decoration">プロジェクト</span>
          <span class="[writing-mode:vertical-lr] text-lg font-display">Projects</span>
        </div>
        ...
        <nav class="hidden lg:flex flex-col gap-4 mt-12 text-sm font-display" aria-label="Page sections">
          <a
            ...
            :class="activeId === 'overview' ? 'text-white' : 'text-white/40 hover:text-white'"
          >
            <span class="h-px transition-all duration-300" :class="activeId === 'overview' ? 'w-12 bg-white' : 'w-8 bg-white/20 group-hover:w-12'"></span>
            Overview
          </a>
          <a
            href="#filter"
            class="transition-colors flex items-center gap-3 group uppercase tracking-widest"
            :class="activeId === 'filter' ? 'text-white' : 'text-white/40 hover:text-white'"
          >
            <span class="h-px transition-all duration-300" :class="activeId === 'filter' ? 'w-12 bg-white' : 'w-8 bg-white/20 group-hover:w-12'"></span>
            Filter
          </a>
          <a
            href="#showcase"
            class="transition-colors flex items-center gap-3 group uppercase tracking-widest"
            :class="activeId === 'showcase' ? 'text-white' : 'text-white/40 hover:text-white'"
          >
            <span class="h-px transition-all duration-300" :class="activeId === 'showcase' ? 'w-12 bg-white' : 'w-8 bg-white/20 group-hover:w-12'"></span>
            Showcase
          </a>
        </nav>
      </div>
    ...
    <main class="w-full lg:min-w-5xl max-w-5xl space-y-12 pb-32 mx-auto">
      <section id="overview" class="relative group">
        ...
        <h2 class="text-xs font-display text-white/40 uppercase tracking-widest mb-12 pl-4">01 / Overview</h2>

        <div class="space-y-8">
          <div class="flex items-center gap-4 mb-8">
            <div class="h-px flex-1 bg-white/10"></div>
            <span class="text-6xl font-display text-white/10 font-bold">01</span>
          </div>
          ...
          <div class="space-y-2 pl-1">
            ...
            <p class="text-xs text-white/40 max-w-xs font-display leading-relaxed">
              Production systems, internal tools, and open-source contributions. Each project solved real problems with measurable impact.
            </p>
          </div>

          <div class="grid grid-cols-3 gap-6 pt-8">
            <div class="bg-white/5 backdrop-blur-sm rounded-xl p-4 md:p-6 border border-white/10 flex flex-col justify-between">
              <div class="text-4xl font-display font-bold text-white mb-2">{{ projectStats.total }}</div>
              <div class="text-xs md:text-sm font-display text-white/60">Total Projects</div>
            </div>
            <div class="bg-white/5 backdrop-blur-sm rounded-xl p-4 md:p-6 border border-white/10 flex flex-col justify-between">
              <div class="text-4xl font-display font-bold text-white mb-2">{{ projectStats.featured }}</div>
              <div class="text-xs md:text-sm font-display text-white/60">Featured</div>
            </div>
            <div class="bg-white/5 backdrop-blur-sm rounded-xl p-4 md:p-6 border border-white/10 flex flex-col justify-between">
              <div class="text-4xl font-display font-bold text-white mb-2">{{ projectStats.completed }}</div>
              <div class="text-xs md:text-sm font-display text-white/60">Completed</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="filter" class="relative group">
        <div class="absolute -left-4 top-0 bottom-0 w-px bg-white/10 origin-top scale-y-0 transition-transform group-hover:scale-y-100 duration-500"></div>
        <h2 class="text-xs font-display text-white/40 uppercase tracking-widest mb-8 pl-4">02 / Filter</h2>

        <div class="space-y-6">
          <div>
            <p class="text-sm font-display text-white/60 mb-3 uppercase tracking-wider">Project Type</p>
          ...
          <div>
            <div class="flex items-center justify-between mb-3">
              <p class="text-sm font-display text-white/60 uppercase tracking-wider">Technology Stack</p>
              <button
                v-if="selectedTech"
            ...
            <div class="flex flex-wrap gap-2">
              <button
                ...
                :class="selectedTech === tech ? 'bg-accent text-white border border-white/30' : 'bg-accent/30 text-white/80 border border-white/5 hover:bg-accent/50'"
              >
                {{ tech }}
              </button>
            </div>
          ...
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-display text-white/60">Active Filters:</span>
              ...
              <span class="text-sm font-display text-white/40">{{ filteredProjects.length }} results</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="showcase" class="relative group">
        <div class="absolute -left-4 top-0 bottom-0 w-px bg-white/10 origin-top scale-y-0 transition-transform group-hover:scale-y-100 duration-500"></div>
        <h2 class="text-xs font-display text-white/40 uppercase tracking-widest mb-8 pl-4">03 / Showcase</h2>
        ...
        >
          ...
          <p class="text-sm font-display text-white/60">
            ...
            <button @click="clearAllFilters" class="text-white underline hover:text-white/80">
              clear all filters
            </button>
          </p>
        </div>
      ...
      <section class="relative group">
        ...
        <div class="bg-white/5 backdrop-blur-sm rounded-2xl p-8 border border-white/10">
          <h3 class="text-2xl font-display font-bold text-white mb-4">Want to Build Something?</h3>

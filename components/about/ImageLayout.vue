...
watch(
  ...
  (val) => {
    ...
    if (val) {
      // Force reset semua element sebelum in animation
      gsap.set("#image-0", { autoAlpha: 0, x: -50 });
      gsap.set("#image-1", { autoAlpha: 0, y: -50 });
      gsap.set("#image-2", { autoAlpha: 0, y: 50 });

      outTimeline.pause(0);
      inTimeline.play(0); // play from start instead of restart
    } else {
      inTimeline.pause(0);
      outTimeline.restart();
    }
